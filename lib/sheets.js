...
export const createGoogleSheetsTemplate = async (user) => {
  try {
    ...
    try {
      ...
      // First, try to get the resource key for the template
      let resourceKey = null;
      try {
        console.log('📊 Fetching template metadata to check for resource key...');
        const fileMetadata = await drive.files.get({
          fileId: TEMPLATE_ID,
          fields: 'id,name,resourceKey,webViewLink',
          supportsAllDrives: true
        });
        
        resourceKey = fileMetadata.data.resourceKey;
        if (resourceKey) {
          console.log('📊 Resource key found:', resourceKey);
        } else {
          console.log('📊 No resource key required for this template');
        }
        
        // Also log the webViewLink to check if it contains a resource key
        if (fileMetadata.data.webViewLink) {
          console.log('📊 Template web link:', fileMetadata.data.webViewLink);
        }
      } catch (metadataError) {
        console.log('📊 Could not fetch template metadata:', metadataError.message);
        // Continue anyway - the template might still be copyable
      }
      ...
      // Add resource key header if available
      if (resourceKey) {
        copyRequest.headers = {
          'X-Goog-Drive-Resource-Keys': `${TEMPLATE_ID}/${resourceKey}`
        };
        console.log('📊 Adding resource key to copy request');
      }
...
export const syncBetToSheets = async (user, betData) => {
  try {
    ...
    let matchDateTime = '';
    if (betData.match_datetime) {
      matchDateTime = betData.match_datetime;
    } else if (betData.date) {
      // If date is in format like "08-31" or "31/08", add current year
      const dateStr = betData.date;
      if (dateStr.includes('-') || dateStr.includes('/')) {
        const currentYear = new Date().getFullYear();
        // Try to parse the date with current year
        const parsedDate = new Date(`${currentYear}-${dateStr.replace('/', '-')}`);
        if (!isNaN(parsedDate)) {
          // Format as YYYY-MM-DD HH:MM for consistency
          matchDateTime = parsedDate.toISOString().split('T')[0] + ' 00:00';
        } else {
          matchDateTime = dateStr; // Fallback to original
        }
      } else {
        matchDateTime = dateStr;
      }
    } else {
      // Use current date/time as fallback
      matchDateTime = now.toISOString().split('T')[0] + ' ' + now.toTimeString().split(' ')[0].slice(0, 5);
    }
    ...
    // Format stake with currency symbol if needed
    const stakeFormatted = stake ? `${stake} €` : '';
    
    const rowData = [
      // ID column - leave empty, will be auto-generated by formula in template
      '', 
      // Status - use exact values that match dropdown validation
      betData.status?.toUpperCase() || 'PENDING',
      // Match DateTime - properly formatted date
      matchDateTime,
      // Sport - lowercase to match typical sports values
      (betData.sport || '').toLowerCase(),
      // Teams/Event
      betData.teams || betData.event || '',
      // Market - should match dropdown values in template
      betData.market || betData.bet_type || '',
      // Selection
      betData.selection || '',
      // Odds - as number
      betData.odds || '',
      // Stake - formatted with currency
      stakeFormatted,
      // To Win - calculated value with currency
      toWin ? `${toWin} €` : '',
      // Return - for settled bets
      betData.actual_return || '',
      // P/L - leave empty, calculated by formula
      '',
      // Bookmaker
      betData.bookmaker || '',
      // Type - match dropdown values
      betData.is_live ? 'LIVE' : 'PRE-MATCH',
      // Added - current date/time
      now.toISOString().split('T')[0] + ' ' + now.toTimeString().split(' ')[0].slice(0, 5),
      // Days Pending - leave empty, calculated by formula
      '',
    ...
    console.log('📊 Finding next empty row in spreadsheet...');
    let nextRow = 2; // Start checking from row 2 (after header)
    
    try {
      const currentData = await sheets.spreadsheets.values.get({
        spreadsheetId: user.spreadsheet_id,
        range: 'Bet Tracker!A2:A1000', // Check ID column for next empty row
        valueRenderOption: 'UNFORMATTED_VALUE'
      });
      
      if (currentData.data.values) {
        // Find the first empty row (accounting for auto-generated IDs)
        // We'll check column B (Status) since ID is auto-generated
        const statusData = await sheets.spreadsheets.values.get({
          spreadsheetId: user.spreadsheet_id,
          range: 'Bet Tracker!B2:B1000',
          valueRenderOption: 'UNFORMATTED_VALUE'
        });
        
        if (statusData.data.values) {
          nextRow = statusData.data.values.length + 2; // +2 because we start from row 2
        }
      }
    } catch (err) {
      console.log('📊 No existing data found, using row 2');
    }
    
    console.log(`📊 Inserting data at row ${nextRow}...`);
    
    // First, we need to copy the formatting and validation from row 2 (template row) to the new row
    // This ensures dropdowns and formatting are preserved
    try {
      // Get the sheet ID for the Bet Tracker sheet
      const spreadsheetInfo = await sheets.spreadsheets.get({
        spreadsheetId: user.spreadsheet_id,
        fields: 'sheets(properties)'
      });
      
      const betTrackerSheet = spreadsheetInfo.data.sheets.find(
        sheet => sheet.properties.title === 'Bet Tracker'
      );
      
      if (betTrackerSheet) {
        const sheetId = betTrackerSheet.properties.sheetId;
        
        // Copy formatting and data validation from row 2 to the new row
        const requests = [
          // Copy all formatting from row 2 to the new row
          {
            copyPaste: {
              source: {
                sheetId: sheetId,
          ...
          // Copy data validation rules
          {
            copyPaste: {
              ...
              pasteType: 'PASTE_DATA_VALIDATION' // Copy validation rules (dropdowns)
            }
          }
        ];
        
        // Apply the formatting and validation
        await sheets.spreadsheets.batchUpdate({
          spreadsheetId: user.spreadsheet_id,
          resource: { requests }
        });
        
        console.log('📊 Formatting and validation copied to new row');
      }
    } catch (formatError) {
      console.log('⚠️ Could not copy formatting to new row:', formatError.message);
      // Continue anyway - data will still be inserted
    }
    
    // Now update the row with the actual data
    const updateResult = await sheets.spreadsheets.values.update({
      spreadsheetId: user.spreadsheet_id,
      range: `Bet Tracker!A${nextRow}:Q${nextRow}`, // Specific row range
      valueInputOption: 'USER_ENTERED',
      resource: { values: [rowData] }
    });
    
    console.log('📊 Google Sheets update successful:', updateResult.data);
